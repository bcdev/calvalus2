    ...
    for j in range(i+1,len(files)):
        ...
        if start(files[j]) > start(files[i]) and stop(files[j]) > stop(files[i]):
            if stop(files[i]) - start(files[i]) > stop(files[j]) - start(files[j]):
                print files[j], files[i], stop(files[i]) - start(files[j]), stop(files[j]) - stop(files[i])
            else:
                print files[i], files[j], stop(files[i]) - start(files[j]), start(files[j]) - start(files[i])
