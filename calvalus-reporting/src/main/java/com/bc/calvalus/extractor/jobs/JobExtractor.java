package com.bc.calvalus.extractor.jobs;
 ...
 * @author muhammad.bc.
 */
public class JobExtractor extends com.bc.calvalus.extractor.Extractor {
    ...
    private JobsType getAllPresentJobDetails(String sourceUrl) {
        try {
            Reader reader = getReadFromSource(sourceUrl, com.bc.calvalus.extractor.ReadFormatType.XML);
    ...
    private JobType getJobDetail(String sourceUrl) {
        Reader reader = getReadFromSource(sourceUrl, com.bc.calvalus.extractor.ReadFormatType.JSON);
        Gson gson = new Gson();
