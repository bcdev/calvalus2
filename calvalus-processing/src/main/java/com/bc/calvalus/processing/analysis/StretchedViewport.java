...
public class StretchedViewport implements Viewport {
    ...
    public StretchedViewport() {
        this(new Rectangle());
    }

    public StretchedViewport(Rectangle viewBounds) {
        this(viewBounds, false);
    }

    public StretchedViewport(boolean modelYAxisDown) {
        this(new Rectangle(), modelYAxisDown);
    }

    public StretchedViewport(Rectangle viewBounds, boolean modelYAxisDown) {
    ...
    public void zoom(Rectangle2D modelBounds) {
        ...
        final double zoomFactorX = viewportWidth / modelBounds.getWidth();
        final double zoomFactorY = viewportHeight / modelBounds.getHeight();
        if (zoomFactorX > 0.0 && zoomFactorY > 0.0) {
            setZoomFactor(zoomFactorX, zoomFactorY, modelBounds.getCenterX(), modelBounds.getCenterY());
...
//        System.out.println("  computedViewBounds  = " + getModelToViewTransform().createTransformedShape(modelBounds).getBounds2D());
    ...
    protected void setZoomFactor(double zoomFactorX, double zoomFactorY, double modelCenterX, double modelCenterY) {
        Assert.argument(zoomFactorX > 0.0, "zoomFactorX > 0.0");
        Assert.argument(zoomFactorY > 0.0, "zoomFactorY > 0.0");
        final double sx = 1.0;
        final double sy = modelYAxisDown ? 1.0 : -1.0;
        final double viewportWidth = viewBounds.width;
        final double viewportHeight = viewBounds.height;
        final double modelOffsetX = modelCenterX - 0.5 * sx * viewportWidth / zoomFactorX;
        final double modelOffsetY = modelCenterY - 0.5 * sy * viewportHeight / zoomFactorY;
        ...
        m2v.scale(zoomFactorX, zoomFactorY);
    ...
    public Viewport clone() {
        try {
            StretchedViewport vp = (StretchedViewport) super.clone();
