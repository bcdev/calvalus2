 ...
 * Call with:
 * <pre>
 *    hadoop jar target/calvalus-experiments-1.3-SNAPSHOT-job.jar \
...
public class ExecutablesTool extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        try {
            ...
            String pathname = "lib/calvalus-processing-1.3-SNAPSHOT-job.jar";
            if (!new File(pathname).exists()) {
                pathname = "calvalus-processing/target/calvalus-processing-1.3-SNAPSHOT-job.jar";
                if (!new File(pathname).exists()) {
