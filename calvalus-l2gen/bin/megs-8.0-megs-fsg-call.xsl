...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="calvalus.expected.filename">MER_FSG_2P<xsl:value-of select="substring($calvalus.input.filename,11)" /></xsl:variable>

  <!-- constructs call with env script, executable, input, output, move of temporal output to target dir -->

  <xsl:template match="/">

    echo "looking for expected output <xsl:value-of select="$calvalus.expected.filename"/>"
    hadoop fs -ls <xsl:value-of select="$calvalus.output" />/<xsl:value-of
          select="$calvalus.input.year" />/<xsl:value-of
          select="$calvalus.input.month" />/<xsl:value-of
          select="$calvalus.input.day" />/<xsl:value-of select="$calvalus.expected.filename"/> 2&gt; /dev/null
    if [ $? = 0 ]; then
      hadoop fs -touchz <xsl:value-of select="$calvalus.output" />/<xsl:value-of
          select="$calvalus.input.year" />/<xsl:value-of
          select="$calvalus.input.month" />/<xsl:value-of
          select="$calvalus.input.day" />/<xsl:value-of select="$calvalus.expected.filename"/>.valid
      echo "<xsl:value-of select="$calvalus.expected.filename"/> exists"
      exit 0
    fi
...
hadoop fs -get <xsl:value-of select="$calvalus.input" /><xsl:text> </xsl:text><xsl:value-of select="$calvalus.tmp.dir" />/files/l1_fsg.prd
...
outputfile=`head -1 files/l2_fsg.prd | cut -d'=' -f2 | sed s/\"//g`
#outnetcdf=`head -1 files/l2_fsg.prd | cut -d'=' -f2 | sed s/\"//g | sed 's/\(.*\).../\1/'`.nc
#mv files/output.nc $outnetcdf
configfile=modifiers_`head -1 files/l2_fsg.prd | cut -d'=' -f2 | sed s/\"//g | sed 's/\(.*\).../\1/'`.db
#netcdfconfigfile=netcdf_`head -1 files/l2_fsg.prd | cut -d'=' -f2 | sed s/\"//g | sed 's/\(.*\).../\1/'`.db
mv files/l2_fsg.prd $outputfile
...
if test -e <xsl:value-of select="$calvalus.tmp.dir" />/*.N1 ; then
  ...
  hadoop fs -touchz <xsl:value-of select="$calvalus.output" />/<xsl:value-of
        select="$calvalus.input.year" />/<xsl:value-of
        select="$calvalus.input.month" />/<xsl:value-of
        select="$calvalus.input.day" />/${outputfile}.valid
fi
