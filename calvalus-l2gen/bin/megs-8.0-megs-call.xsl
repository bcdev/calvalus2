...
hadoop fs -get <xsl:value-of select="$calvalus.input" /><xsl:text> </xsl:text><xsl:value-of select="$calvalus.tmp.dir" />/files/l1_rr.prd
...
# netcdf file as well
outnetcdf=`head -1 files/l2_rr.prd | cut -d'=' -f2 | sed s/\"//g | sed 's/\(.*\).../\1/'`.nc
mv files/output.nc $outnetcdf

# rename configuration files

configfile=modifiers_`head -1 files/l2_rr.prd | cut -d'=' -f2 | sed s/\"//g | sed 's/\(.*\).../\1/'`.db

netcdfconfigfile=netcdf_`head -1 files/l2_rr.prd | cut -d'=' -f2 | sed s/\"//g | sed 's/\(.*\).../\1/'`.db
