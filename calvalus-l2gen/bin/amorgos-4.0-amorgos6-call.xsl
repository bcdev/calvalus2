...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="calvalus.package.dir">/home/hadoop/opt/amorgos-4.0</xsl:param>
...
lookup_output() {
  [ "$1" != "" ] &amp;&amp; \
  o=`hadoop fs -ls <xsl:value-of select="$calvalus.output" />/<xsl:value-of
          select="$calvalus.input.year" />/<xsl:value-of
          select="$calvalus.input.month" />/<xsl:value-of
          select="$calvalus.input.day" />/$1 2&gt; /dev/null`
  if [ $? = 0 ]; then
    f=`basename "$o"`
    ...
    hadoop fs -touchz <xsl:value-of select="$calvalus.output" />/<xsl:value-of
          select="$calvalus.input.year" />/<xsl:value-of
          select="$calvalus.input.month" />/<xsl:value-of
          select="$calvalus.input.day" />/${f}.valid
    return 0
  else
    return 1
  fi
...
if [ "$input" = "" ]; then
  if ! output_exists $input 1 $length; then
    echo "$input not listed, will be processed completely"
    do_process
    code=$?
  fi
else
